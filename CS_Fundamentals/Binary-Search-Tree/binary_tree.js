...
class BinaryTree{
  ...
  remove(value, currentNode = this.root) {
    ...
    if(currentNode.value === value){
      ...
      currentNode = this.#removeNodeHelper(value, currentNode.leftChild);      
    }else{
      currentNode = this.#removeNodeHelper(value, currentNode.rightChild);
    }
    return currentNode;
  }
  find(value, node = this.root) {
   if(node === null || node.value === null) return node;
  ...
  levelOrder(callbackFn){
   ...
   while(queue.length > 0){
      ...
      const enqueueList = [currentNode?.leftChild, currentNode?.rightChild].filter((value) => value);
  ...
  #testBalance(node){
   ...
   if(leftBalance === -1 || rightBalance === -1 || diff > 1){
      ...
      return Math.max(rightBalance, leftBalance) + 1;
   }
  }

  #inOrderSuccesFor(node){
  ...
  #removeNodeHelper(node) {
   if(node.leftChild && node.rightChild){
      const successorNode = this.#inOrderSuccesFor(node.rightChild);
